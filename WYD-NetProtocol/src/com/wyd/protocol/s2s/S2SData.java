...
//    private static final String sep = ", ";
    ...
    public boolean[] readBooleans() throws IllegalAccessException {
        if ((data[pos] & 0xff) != 130) throw new IllegalAccessException();
        pos++;
        int len = (int) getNumber(data, pos, 2);
        pos += 2;
        if (pos + len > data.length) throw new IllegalAccessException();
    ...
    public byte[] readBytes() throws IllegalAccessException {
        if ((data[pos] & 0xff) != 129) throw new IllegalAccessException();
        pos++;
    ...
    public short[] readShorts() throws IllegalAccessException {
        if ((data[pos] & 0xff) != 131) throw new IllegalAccessException();
        pos++;
        int len = (int) getNumber(data, pos, 2);
        pos += 2;
        if (pos + len * 2 > data.length) throw new IllegalAccessException();
    ...
    public int[] readInts() throws IllegalAccessException {
        if (pos >= data.length - 1 || (data[pos] & 0xff) != 132) throw new IllegalAccessException();
        pos++;
        int len = (int) getNumber(data, pos, 2);
        pos += 2;
        if (pos + len * 4 > data.length) throw new IllegalAccessException();
        int ret[] = new int[len];
        for (int i = 0; i < len; i++) {
    ...
    public long[] readLongs() throws IllegalAccessException {
        if ((data[pos] & 0xff) != 133) throw new IllegalAccessException();
        pos++;
        int len = (int) getNumber(data, pos, 2);
        pos += 2;
        if (pos + len * 8 > data.length) throw new IllegalAccessException();
    ...
    public String readString() throws IllegalAccessException {
        byte md = data[pos];
        if (pos >= data.length - 1 || md != 6) throw new IllegalAccessException();
        pos++;
        int len = (int) getNumber(data, pos, 2);
        pos += 2;
        if (pos + len > data.length) throw new IllegalAccessException();
    ...
    public String[] readStrings() throws IllegalAccessException {
        if ((data[pos] & 0xff) != 134) throw new IllegalAccessException();
        pos++;
