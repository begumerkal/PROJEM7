...
public class DataBeanEncoder {
	...
	public INetSegment encode(AbstractData data) throws Exception {
		...
		for (Field f : fs) {
			...
			if (value == null) {
				System.out.println("type:" + data.getType() + "------------SubType:" + data.getSubType() + " PropertyName:" + f.getName()
						+ " value is null");
			} else if (value instanceof String[]) {
				for (String str : (String[]) value) {
					if (str == null) {
						System.out.println("type:" + data.getType() + "------------SubType:" + data.getSubType() + " PropertyName:"
								+ f.getName() + " value is null");
					}
				}
			}
