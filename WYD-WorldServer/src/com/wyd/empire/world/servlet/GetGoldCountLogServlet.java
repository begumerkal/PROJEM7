...
public class GetGoldCountLogServlet extends HttpServlet {
	private static final long serialVersionUID = -4641583253543697220L;
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			PageList pageList = ServiceManager.getManager().getGetLogService().getGoldCountLogList(sb.toString(), pageIndex, pageSize);
			...
			if (null != pageList && pageList.getList().size() > 0) {
				...
				for (Object obj : pageList.getList()) {
					GoldCount goldCount = (GoldCount) obj;
					Properties p = new Properties();
					p.put("id", goldCount.getId());
					p.put("amount", goldCount.getAmount());
					p.put("playerId", goldCount.getPlayerId());
					p.put("origin", goldCount.getOrigin());
					p.put("remark", goldCount.getRemark() == null ? "" : goldCount.getRemark());
					p.put("createTime", goldCount.getCreateTime());
					mailList.add(p);
				}
