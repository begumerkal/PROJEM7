...
public class CrossPlayerInfoHandler implements IDataHandler {
	...
	public void handle(AbstractData message) throws Exception {
		...
		try {
			int playerId = ServiceManager.getManager().getCrossService().getPlayerId(cce.getFriendId());
			WorldPlayer player = ServiceManager.getManager().getPlayerService().getWorldPlayerById(playerId);
			if (null == player) {
				return;
			}
			CrossPlayerInfoOk playerInfo = new CrossPlayerInfoOk();
			playerInfo.setBattleId(cce.getBattleId());
			playerInfo.setPlayerId(cce.getPlayerId());
			playerInfo.setFriendId(cce.getFriendId());
			...
			playerInfo.setIsFriend(isFriend);
			List<PlayerItemsFromShop> playerItemList = getPlayerItem(playerId);
			...
			ServiceManager.getManager().getCrossService().send(playerInfo);
		} catch (Exception ex) {
			ex.printStackTrace();
			log.error(ex, ex);
		}
	...
	private void setPictureUrl(PlayerPicture privateInfo, CrossPlayerInfoOk playerInfo) {
	...
	// 公会
	private void setGuild(WorldPlayer player, CrossPlayerInfoOk playerInfo) {
	...
	// 微博信息
	private void setWeibo(WorldPlayer player, CrossPlayerInfoOk playerInfo) {
