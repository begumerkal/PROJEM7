...
import com.wyd.empire.protocol.data.player.GetPlayerPropOk;
 ...
 *
 */
public class GetPlayerPropHandler implements IDataHandler {
	Logger log = Logger.getLogger(GetPlayerPropHandler.class);
	...
	public void handle(AbstractData data) throws Exception {
		...
		try {
			...
			GetPlayerPropOk getPlayerPropOk = new GetPlayerPropOk(data.getSessionId(), data.getSerial());
			getPlayerPropOk.setCount(count);
			getPlayerPropOk.setId(id);
			getPlayerPropOk.setName(name);
			getPlayerPropOk.setIcon(icon);
			getPlayerPropOk.setPriceCostGold(priceCostGold);
			getPlayerPropOk.setDesc(desc);
			getPlayerPropOk.setMainType(mainType);
			getPlayerPropOk.setIsSubType(isSubType);
			getPlayerPropOk.setParam1(param1);
			getPlayerPropOk.setParam2(param2);
			getPlayerPropOk.setTireValue(tireValue);
			getPlayerPropOk.setConsumePower(consumePower);
			getPlayerPropOk.setSpecialAttackType(specialAttackType);
			getPlayerPropOk.setSpecialAttackParam(specialAttackParam);

			session.write(getPlayerPropOk);
		} catch (Exception ex) {
			if (null == ex.getMessage() || !ex.getMessage().startsWith(Common.ERRORKEY))
				this.log.error(ex, ex);
			throw new ProtocolException(ErrorMessages.PLAYER_GUPF_MESSAGE, data.getSerial(), data.getSessionId(), data.getType(),
					data.getSubType());
		}
	}
}
