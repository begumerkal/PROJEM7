...
public class SimpleWYDDecoderForSingleSocket extends ProtocolDecoderAdapter {
	@SuppressWarnings("unused")
	private static final Logger log = Logger.getLogger(SimpleWYDDecoderForSingleSocket.class);
	...
	public void decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
		...
		while (buffer.hasRemaining()) {
			...
			if (session.getAttribute("REMOTE_PROXY_PORT") == null) {
				short port = buffer.getShort();
				session.setAttribute("REMOTE_PROXY_PORT", Short.valueOf(port));
				out.write(Short.valueOf(port));
			} else if (size > 18) {
