...
public class BattleTeam {
    ...
    private int                                battleMode;                                      // 战斗模式,1、竞技模式，2、复活模式，3、混战模式，4、排位赛，5公会战
    ...
    private CrossPostionsForPlayers            postionsForPlayers;                              // 玩家所在地图初始位置
    private boolean                            newRun;                                          // 是否新回合
    private boolean                            over              = false;                       // 该对战是否已结束
    private int                                loadRoom          = 0;                           // 加载的房间数量
    private List<SessionInfo>                  sessionList       = new ArrayList<SessionInfo>();
    private int[]                              battleRand;                                      // 游戏随机数
    private int                                critValue         = 0;                           // 暴击值
    ...
    public synchronized int getLoadRoom() {
        return loadRoom++;
    }
    ...
    public void enter(Combat combat) {
        ...
        combatMap.put(combat.getPlayer().getId(), combat);
    }
    ...
    public Combat getActionPlayer() {
        ...
        // ServiceManager.getManager().getBattleTeamService().sendSort(battleId, combat.getPlayer().getId());
        return combat;
    }
    ...
    public CrossPostionsForPlayers getPostionsForPlayers() {
        return postionsForPlayers;
    }

    public void setPostionsForPlayers(CrossPostionsForPlayers postionsForPlayers) {
    ...
    public void addSession(AcceptSession session, int roomId) {
        sessionList.add(new SessionInfo(session, roomId));
    }
     ...
     * @param data
     */
    public void sendData(CrossDate data) {
        for (SessionInfo session : sessionList) {
            data.setRoomId(session.getRoomId());
            session.getSession().send(data);
        }
    }
    class SessionInfo {
        ...
        public SessionInfo(AcceptSession session, int roomId) {
            this.session = session;
            this.roomId = roomId;
        }

        public AcceptSession getSession() {
            return session;
        }
