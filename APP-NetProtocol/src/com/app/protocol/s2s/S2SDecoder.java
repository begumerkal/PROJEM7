			...
			if (size > 19) {
				...
				if (len + 1 <= size) {
					...
					for (int i = 0; i < num; i++) {
						...
						// log.info("MainType: " + maintype + "=>Subtype" +
						// subtype + "=>Remaining: " + l);
					...
					buffer.get();
					if ((flag & 1) != 0)
						log.debug((new StringBuilder()).append("***Recv Error Msg:").append(datas[0].getType()).append(".")
								.append(datas[0].getSubType()).toString());
					else
						log.debug((new StringBuilder()).append("***Recv Protocol:").append(datas[0].getType()).append(".")
								.append(datas[0].getSubType()).toString());
