...
public class ExchangeServlet extends HttpServlet {
    ...
    private static final Logger log            = Logger.getLogger(ExchangeServlet.class);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (check) {
                ...
                if (null != codeExchange && codeExchange.getCode().equals(exchangeInfo.getCode())) {
                    if (codeExchange.getChannel_id().equals("-1") || codeExchange.getChannel_id().equals(String.valueOf(exchangeInfo.getChannel()))) {
                        if (codeExchange.getMinlevel() <= exchangeInfo.getLevel() && exchangeInfo.getLevel() <= codeExchange.getMaxlevel()) {
                            int count = ServiceManager.getManager().getExchangeService().getExchangeCount(exchangeInfo.getPlayerId(), exchangeInfo.getServiceId(), codeExchange.getBatchNum());
                            if (count == 0) {
                                if (DateUtil.compareDateOnDay(new Date(), codeExchange.getExtendtime()) <= codeExchange.getUseful_life()) {
                                    if (codeExchange.getUsed() == 0) {
                                        if (codeExchange.getExtended() == 1) {
                                            exchangeResult.setCode(0);
                                            exchangeResult.setMessage(codeExchange.getContent());
                                            exchangeResult.setRewardType(codeExchange.getRewardType());
                                            codeExchange.setUsed(1);
                                ...
                                exchangeResult.setCode(-1);
                                exchangeResult.setMessage("玩家已领取过该批次奖励");
                            }
                        } else {
                            exchangeResult.setCode(6);
                            exchangeResult.setMessage("不符合领取该礼包的等级需求");
                        }
                    } else {
